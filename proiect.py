...
class Hangman:
     ...
     def __init__(self):
          with open("word_list.txt", "r") as f:
               for word in f.read().split('\n'):
                    self.__word_list.append(word.lower())
     ...
     def StartGame(self):
          ...
          while not gameOver:
               ...
               ch.append(input("Your desired character is: ").lower())
               ...
               while True:
                    ...
                    if ch[-1] not in ch[:-1] and len(ch) > 0 and not(ch[-1]>='0' and ch[-1]<='9') and not(len(ch[-1])>1) and (ch[-1]>='a' and ch[-1]<='z'): # if user input character is a new one (OK).
                         break
                    else: # is already used then remove and try again (BAD).
                         if ch[-1]>='0' and ch[-1]<='9':
                            ch.pop(-1) # remove last input from list.
                            ch.append(input("You can't introduce a number! Try a new one: ").lower())
                         elif len(ch[-1])>1:
                            ch.pop(-1) # remove last input from list.
                            ch.append(input("You can't introduce more than one character! Try a new one: ").lower())
                         elif ch[-1]<'a' or ch[-1]>'z':
                            ch.pop(-1) # remove last input from list.
                            ch.append(input("Character introduced is not a letter! Try a new one: ").lower())
                         else:
                            ch.pop(-1) # remove last input from list.
                            ch.append(input("Character used before! Try a new one: ").lower())
     ...
     def CheckWin(self, winner):
          ...
          if winner == 0:
               print(f"You lost, {self._nickname}! The word was: {self.magic_word}")
          if winner > 0:
...
if __name__ == '__main__':
     ...
     hInstance.NewGame(username) # Set your name as parameter.